...
// ── Progreso / bienestar ──────────────────────────────────────────────
// Se calcula SOLO con datos reales; si una sección no tiene nada, ese arco
// no se dibuja. Vive acá (compartido) porque lo usan Home (semáforo) y
// Perfil (anillo + análisis).
...
export function datosBienestar() {
...
// Racha: días consecutivos (terminando hoy) con al menos un gasto registrado.
export function rachaDeGastos(): number {
  const g = loadV2GastosState<GastosLite>();
  if (!g) return 0;
...
export function Arco({ radius, pct, color }: { radius: number; pct: number | null; color: string }) {
...
export function AnalisisCard({ titulo, valor, sub, color }: { titulo: string; valor: string; sub: string; color: string }) {
  return (
